...
public class TableDataSource: NSObject, UITableViewDataSource, UITableViewDelegate{
    ...
    init(controller: UIViewController? = nil,withData: [GenericTableSection]? = nil, tableView: UITableView,registerCells: UITableViewCell.Type...,isSectionIndexTitleAvail: Bool = false ,selection: CollectionTableSelection? = nil, startScrolling: StartScrolling? = nil, endScrolling: EndScrolling? = nil){
        ...
        self.data = withData
        registerCells.forEach({tableView.registerCell(cells: $0.self)})
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return tableView.cell(tableViewCell: data?[indexPath.section].tableRows?[indexPath.item].tableCell ?? UITableViewCell.self, withCellDetails: data?[indexPath.section].tableRows?[indexPath.item].rowData, controller: controller, indexpath: indexPath)
    }
...
extension TableDataSource{
    ...
    public func sectionIndexTitles(for tableView: UITableView) -> [String]? {
        if isSectionIndexTitleAvail == true && data != nil{
